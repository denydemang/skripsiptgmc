...
return new class extends Migration
{
    ...
    {
        Schema::create('stocksavg', function (Blueprint $table) {
            $table->bigIncrements("id");
            $table->string("ref_no", 100);
            $table->string("item_code", 50);
            $table->string("unit_code", 50);
            $table->dateTime("item_date");
            $table->decimal("actual_stock", 60,4);
            $table->decimal("used_stock", 60,4);
            $table->decimal("total_in", 60,4);
            $table->decimal("total_out", 60,4);
