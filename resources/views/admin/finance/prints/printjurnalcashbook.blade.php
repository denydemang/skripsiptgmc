...
<head>
	...
	<title>Journal CashBook {{ $cashbook['cash_no'] }}</title>
	<style>
		body {
...
<body>
	...
	<div class="header-container" style="position: relative;padding-top:120px;border-bottom:2px solid #333">
		...
		<header style="position:absolute;top:0;left:0;padding-top:40px">
			<h3 style="margin-top:-10px;text-align:left">JOURNAL CASHBOOK</h3>
		</header>
	</div>
		...
		<div class="transaction-info">
			<table border="0" cellpadding="4" style="padding: 10px">
				<tr>
					...
					<td>:</td>
					<td>{{ \Carbon\Carbon::parse($cashbook['transaction_date'])->format('d/m/Y') }}</td>
				</tr>
				<tr>
					<td><strong>Cash No</strong></td>
					<td>:</td>
					<td>{{ $cashbook['cash_no'] }}</td>
				</tr>
				<tr>
					<td><strong>Type</strong></td>
					<td>:</td>
					<td>{{ $cashbook['CbpType'] == 'P' ? 'Payment' : 'Receive' }}</td>
					</td>
				</tr>
			</table>
		</div>
			...
			<tbody>
				...
				@foreach ($journal as $coa)
					...
					<tr>
						...
						<td class="no-wrap" style="text-align:right">Rp. {{ number_format(round($coa->debit), 2, ',', '.') }}</td>
						<td class="no-wrap" style="text-align:right">Rp. {{ number_format(round($coa->kredit), 2, ',', '.') }}</td>
					</tr>
				@endforeach
