...
<head>
	...
	<title>Receipt Recapitulation </title>
	<style>
		body {
...
<body>
	...
	<div class="header-container" style="position: relative;padding-top:100px;border-bottom:2px solid #333">
		...
		<header style="position:absolute;top:0;left:0;padding-top:40px">
			<h4 style="margin-top:-10px;text-align:left">RECEIPT RECAPITULATION</h4>
		</header>
	</div>
	<section class="transaction">
		<div class="transaction-info">
			...
			<table border="0" cellpadding="4" style="margin-top:40px;padding: 10px;margin-left:-10px">
				@if ($customercode !== null && count($receipts) > 0)
					<tr>
						<td><strong>Customer Name</strong></td>
						<td>:</td>
						<td>{{ $receipts[0]['customer_name'] }}</td>
					</tr>
					<tr>
						<td><strong>Customer Code</strong></td>
						<td>:</td>
						<td>{{ $receipts[0]['customer_code'] }}</td>
					</tr>
				@endif
				@if ($is_approve !== null && count($receipts) > 0)
					<tr>
				...
				@if (count($receipts) > 0)
					...
					@foreach ($receipts as $p)
						@php
							...
							$totalCashAmount += floatval($p->cash_amount);
						...
						<tr>
							...
							@endif
							<td>{{ $p->received_via }}</td>
							<td class="no-wrap" style="text-align: left">Rp {{ number_format($p->cash_amount, 2, ',', '.') }}</td>
							<td class="no-wrap" style="text-align: left">Rp {{ number_format($p->deposit_amount, 2, ',', '.') }}</td>
							<td class="no-wrap" style="text-align: left">Rp {{ number_format($p->total_amount, 2, ',', '.') }}</td>
						</tr>
					@endforeach
					...
					@endif
					{{-- <td class="no-wrap" colspan="8" style="text-align: right"><b>Total</b></td> --}}
					<td class="no-wrap" style="text-align: left"><b>Rp
							{{ number_format($totalCashAmount, 2, ',', '.') }}</b></td>
					<td class="no-wrap" style="text-align: left"><b>Rp
