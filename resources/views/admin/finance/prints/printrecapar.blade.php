				...
				@if (count($AR) > 0)
					...
					@foreach ($AR as $p)
						...
						<tr>
							...
							<td class="no-wrap" style="text-align: left">Rp {{ number_format($p->deposit_allocation, 2, ',', '.') }}</td>
							<td class="no-wrap" style="text-align: left">Rp
								{{ number_format(floatval($p->deposit_amount) - floatval($p->deposit_allocation), 2, ',', '.') }}</td>
						</tr>
					@endforeach
