...
<head>
	...
	<title>Invoice Recapitulation </title>
	<style>
		body {
...
<body>
	...
	<div class="header-container" style="position: relative;padding-top:100px;border-bottom:2px solid #333">
		...
		<header style="position:absolute;top:0;left:0;padding-top:40px">
			<h4 style="margin-top:-10px;text-align:left">INVOICE RECAPITULATION</h4>
		</header>
	</div>
	<section class="transaction">
		<div class="transaction-info">
			...
			<table border="0" cellpadding="4" style="margin-top:40px;padding: 10px;margin-left:-10px">
				@if ($customercode !== null && count($invoice) > 0)
					<tr>
						<td><strong>Customer Name</strong></td>
						<td>:</td>
						<td>{{ $invoice[0]['customer_code'] }}</td>
					</tr>
					<tr>
				...
				@endif
				@if ($is_approve !== null && count($invoice) > 0)
					<tr>
				...
				@endif
				@if ($paidStatus !== null && count($invoice) > 0)
					<tr>
			...
			<thead>
				<tr>
					...
					@if ($customercode === null)
						...
						<th>Customer Name</th>
					@endif
					<th>Project Realisation Code</th>
					<th>Due Date</th>
					@if ($is_approve === null)
				...
				@if (count($invoice) > 0)
					...
					@foreach ($invoice as $p)
						...
						<tr>
							...
							@if ($customercode === null)
								<td style="text-align: left">{{ $p->customer_code }}</td>
								<td style="text-align: left">{{ $p->customer_name }}</td>
							@endif
							<td>{{ $p->project_realisation_code }}</td>
							<td>{{ $dueDate }}</td>
							@if ($is_approve === null)
					...
					@if ($customercode === null && $is_approve === null)
						<td class="no-wrap" style="text-align: center" colspan="9"> <b>TOTAL</b></td>
