...
<head>
    ...
    <title>Journal Purchase {{ $purchaseData[0]['purchase_no'] }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="header-container" style="position: relative;padding-top:120px;border-bottom:2px solid #333">
        ...
        <header style="position:absolute;top:0;left:0;padding-top:40px">
            <h3 style="margin-top:-10px;text-align:left">JOURNAL PURCHASE</h3>
        </header>
    </div>
        ...
        <div class="transaction-info">
            <table border="0" cellpadding="4" style="padding: 10px">
                <tr>
                    ...
                    <td>:</td>
                    <td>{{ \Carbon\Carbon::parse($purchaseData[0]['transaction_date'])->format('d/m/Y') }}</td>
                </tr>
                <tr>
                    <td><strong>Purchase No</strong></td>
                    <td>:</td>
                    <td>{{ $purchaseData[0]['purchase_no'] }}</td>
                </tr>
                <tr>
                    <td><strong>Supplier Code</strong></td>
                    <td>:</td>
                    <td>{{ $purchaseData[0]['supplier_code'] }}</td>
                    </td>
                </tr>
                <tr>
                    <td><strong>Supplier Name</strong></td>
                    <td>:</td>
                    <td>{{ $purchaseData[0]['supplier_name'] }}</td>
                    </td>
                </tr>
            </table>
        </div>
