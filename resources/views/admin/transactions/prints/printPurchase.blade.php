...
<body>
	...
	<section class="transaction">
		...
		<table id="detail" style="font-size: 12px">
			<thead>
				<tr>
					<th>No</th>
					<th>Description</th>
			...
			<tbody>
				...
				@foreach ($dataPurchase as $x)
					<tr>
						...
						<td>{{ $x->item_name }} ({{ $x->item_code }}) </td>
						<td class="no-wrap">{{ floatval($x->qty) }}</td>
						<td>{{ $x->unit_code }}</td>
						<td class="no-wrap">Rp. {{ number_format($x->price, 2, ',', '.') }}</td>
						<td class="no-wrap">Rp. {{ number_format($x->discount, 2, ',', '.') }}</td>
						<td class="no-wrap">Rp. {{ number_format($x->total, 2, ',', '.') }}</td>
					</tr>
				@endforeach
				<tr>
